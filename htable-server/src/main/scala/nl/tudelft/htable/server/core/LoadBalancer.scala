...
object LoadBalancer {
  ...
  private def reconstruct(nodes: Map[Node, ActorRef[NodeManager.Command]],
                          nodeTablets: mutable.HashMap[Node, Seq[Tablet]]): Behavior[Command] = Behaviors.setup {
    context =>
      implicit val timeout: Timeout = 3.seconds
      ...
      def query(node: Node): Unit = {
        ...
        context.ask(
          nodes(node),
          (ref: ActorRef[NodeManager.ReadResponse]) => NodeManager.Read(Scan("METADATA", RowRange.unbounded), ref)) {
